describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
     ...
     */
    // Get a handle to the first product-item element
    const firstProduct = await page.$('product-item');
 
    // Get the shadowRoot of the product-item (property of the element)
    ...
    // button should say "Remove from Cart"
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    await new Promise(r => setTimeout(r, 1000)); // wait for DOM to update
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    // Get all product-item elements after reload
    ...
    // Also verify cart count is still 20
    const cartCountText = await page.$eval('#cart-count', el => el.innerText);
    expect(cartCountText).toBe('20');
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
     ...
     * Remember to remove the .skip from this it once you are finished writing this test.
     */
    // Use $$eval to click all buttons in one browser-side pass
    await page.$$eval('product-item', (items) => {
